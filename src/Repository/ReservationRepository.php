...
public function getAverageSeats(): float
{
    $result = $this->createQueryBuilder('r')
        ->select('AVG(r.seats) as averageSeats')
        ->getQuery()
...
public function getMostActiveUser(): string
{
    $result = $this->createQueryBuilder('r')
        ->select('u.email, COUNT(r.id) as reservationCount')
        ->join('r.user', 'u')
        ->groupBy('u.email')
    ...
    return $result ? $result['email'] . ' (' . $result['reservationCount'] . ' reservations)' : 'N/A';
}
